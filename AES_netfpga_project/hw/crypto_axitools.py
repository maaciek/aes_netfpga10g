...
def many_byte_xor(buf, key,key_len):
    buf = bytearray(buf)
    orig_key=key
    byte_key=[0 for x in range(key_len)]
    for i in range(0,key_len):
       byte_key[i]=orig_key % 256
       orig_key=orig_key >> 8
    for i, bufbyte in enumerate(buf):
        buf[i] = bufbyte ^ byte_key[i % key_len]
    return str(buf)


def axis_dump( packets, f, bus_width, period, key, tuser_width = 128 , key_width = 32):
    """
    ...
    any .tuser.
    Packets are encrypted using key.
    """
    ...
    if key_width % 8 !=0:
        print "key_width must be a multiple of 8!"
        return
	...
	crypto_payload=str(packet.payload.payload)
        packet.payload.payload=many_byte_xor(crypto_payload,key,key_width/8)
